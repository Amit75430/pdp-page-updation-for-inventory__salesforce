...
import createCase from '@salesforce/apex/contactUsGroup.createCase';
import userId from '@salesforce/user/Id';
//import available from '@salesforce/schema/Inventory__c.Current_Stock__c';
...
export default class B2b_ProductDetailsDisplay extends NavigationMixin(
    ...
    {
        createCase({comment:'Notify us wheneven the product is available',ownerId:userId})
        .then((result)=>
        {
            this.dispatchEvent(
                new ShowToastEvent(
                    {
                        title:'Success',
                        message: 'Donot Worry!!! We will notify with product is available',
                        varient:'Success',
                        mode:'dismissable'
                    }
                )
