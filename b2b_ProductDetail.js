...
import getProduct from '@salesforce/apex/B2B_GetInfoClass.getProduct';
import getCartSummary from '@salesforce/apex/B2B_GetInfoClass.getCartSummary';
import checkProductIsInStock from '@salesforce/apex/B2B_GetInfoClass.checkProductIsInStock';
//import currentdata from '@salesforce/schema/Inventory__c.Current_Stock__c';
import getInventory from '@salesforce/apex/B2B_GetInfoClass.getInventory';
import updateReserved from '@salesforce/apex/B2B_GetInfoClass.updateReserved';
import addToCart from '@salesforce/apex/B2B_GetInfoClass.addToCart';
import createAndAddToList from '@salesforce/apex/B2B_GetInfoClass.createAndAddToList';
import getProductPrice from '@salesforce/apex/B2B_GetInfoClass.getProductPrice';
...
export default class B2b_ProductDetails extends LightningElement {
    ...
    get displayableProduct() {
        return {
            ...
            price: {
                ...
                original:((this.productPrice || {}).data ||{}).listprice
            },
     ...
     */
    addToCart(event) {
        if(event.detail.quantity > this.data.Current_Stock__c)
        {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message: 'Available quantity of the product is ' + this.data.Current_Stock__c + '!!!',
                    variant: 'error',
                    mode: 'dismissable'
                })
            );
        ...
        addToCart({
           ...
           .then(() => {
              // console.log(this.productquantity.data);
                this.dispatchEvent(
                ...
                updateReserved(
                    {   
                        reserved:event.detail.quantity,
                        productId:this.recordId
                    }
                );
            })
